...
<table class="table table-bordered table-hover">
		...
		<?php endfor;?>
			...
			<?php for($i=0;$i<$range;$i++):
				...
				<?php
					if($asist!=null){
						if($asist->kind_id==1){ echo "<i class='fa fa-check text-success'>A</i>"; }
						else if($asist->kind_id==2){ echo "<i class='fa fa-remove text-danger'>F</i>"; }
						else if($asist->kind_id==3){ echo "<span class='text-warning'>R</span>"; }
						else if($asist->kind_id==4){ echo "<span class='text-info'>J</span>"; }
